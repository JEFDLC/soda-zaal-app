...
const dayMeta = [
  { key: "ma", day: "Maandag", jsDay: 1 },
  { key: "di", day: "Dinsdag", jsDay: 2 },
  { key: "wo", day: "Woensdag", jsDay: 3 },
  { key: "do", day: "Donderdag", jsDay: 4 },
  { key: "vr", day: "Vrijdag", jsDay: 5 },
  { key: "za", day: "Zaterdag", jsDay: 6 },
  { key: "zo", day: "Zondag", jsDay: 0 },
];
...
export default function SodaZaalApp() {
  ...
  const todayItem = weekData.find((day) => day.jsDay === todayJsDay) ?? null;

  useEffect(() => {
    if (!todayItem) return;
    const node = dayRefs.current[todayItem.key];
    if (!node) return;
  ...
  return (
    <div className="min-h-screen bg-zinc-100 text-zinc-900">
      <div className="mx-auto flex min-h-screen w-full max-w-md flex-col px-3 pb-24 pt-4">
        <header className="sticky top-0 z-20 mb-3 rounded-3xl bg-white/95 px-4 py-4 shadow-sm ring-1 ring-zinc-200 backdrop-blur">
          ...
          <div className="mt-4 flex items-center gap-2 overflow-x-auto pb-1">
            {todayItem ? (
              <button
                type="button"
                onClick={() => dayRefs.current[todayItem.key]?.scrollIntoView({ behavior: "smooth", block: "start" })}
                className="rounded-full bg-zinc-900 px-3 py-2 text-xs font-semibold text-white whitespace-nowrap"
              >
                Vandaag: {todayItem.day}
              </button>
            ) : null}
            {weekData.map((day) => (
              <button
                key={day.key}
                type="button"
                onClick={() => dayRefs.current[day.key]?.scrollIntoView({ behavior: "smooth", block: "start" })}
                className={`rounded-full px-3 py-2 text-xs font-semibold ring-1 whitespace-nowrap ${
        ...
        <main className="space-y-3">
          {weekData.map((day) => (
            <article
              ...
              className={`overflow-hidden rounded-[28px] bg-white shadow-sm ring-1 ${
                day.jsDay === todayJsDay ? "ring-2 ring-amber-300 shadow-md" : "ring-zinc-200"
              }`}
            >
              <div className="px-4 pb-4 pt-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    {day.jsDay === todayJsDay ? (
                      <span className="rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-amber-900 ring-1 ring-amber-200">
                        Vandaag
                      </span>
                    ) : null}
                  </div>
